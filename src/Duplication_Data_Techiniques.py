...
csv_output_folder = "../output_csv_files/duplicate_results"


def creation_dataset():
    """
    This function provides a dataset with 1000 rows and 5 columns
    with 30% duplicate rows. 
    """
    ...
    df_with_duplicates.to_csv('../data/dataset_with_duplicates.csv', index=False)
...
def remove_duplicates_using_hash(data):
...
def hashing_results(df):
    ...
    data = df.values
    unique_data_custom_hash = remove_duplicates_using_hash(data)
    df = pd.DataFrame(unique_data_custom_hash)
    df.to_csv(f'{csv_output_folder}/data_from_hash.csv', index=False)


def merge_results(df):
    result_merge = merge_sort(df)
    # removing duplicates
    unique_data_custom_hash_merge = remove_duplicates_using_hash(result_merge.values)
    df = pd.DataFrame(unique_data_custom_hash_merge)
    df.to_csv(f'{csv_output_folder}/data_from_hash_merge.csv', index=False)
...
def stack_results(df):
    ...
    df.to_csv(f'{csv_output_folder}/data_from_stack.csv', index=False)
...
def queue_results(df):
    ...
    df.to_csv(f'{csv_output_folder}/data_from_queue.csv', index=False)


def count_results(df):
    result = count_sort(df)
    # removing duplicates
    unique_data_custom_hash_count = remove_duplicates_using_hash(result.values)
    df = pd.DataFrame(unique_data_custom_hash_count)
    df.to_csv(f'{csv_output_folder}/data_from_hash_count.csv', index=False)


def bubble_sort_results(df):
    result = bubble_sort(df)
    # removing duplicates
    unique_data_custom_hash_bubble = remove_duplicates_using_hash(result.values)
    df = pd.DataFrame(unique_data_custom_hash_bubble)
    df.to_csv(f'{csv_output_folder}/data_from_hash_bubble.csv', index=False)


def quick_sort_results(df):
    result = quick_sort(df)
    # removing duplicates
    unique_data_custom_hash_quick = remove_duplicates_using_hash(result.values)
    df = pd.DataFrame(unique_data_custom_hash_quick)
    df.to_csv(f'{csv_output_folder}/data_from_hash_quick.csv', index=False)
...
def main():
    ...
    df_sorting = pd.DataFrame(columns=columns)
    ...
    for algorithm_name, algorithm_func in algorithms.items():
        ...
        df_sorting = pd.concat([df_sorting, new_row], ignore_index=True)

    df_sorting.to_csv('../results/sorting_results_duplication.csv', index=False)
